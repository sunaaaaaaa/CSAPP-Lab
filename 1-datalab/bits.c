 ...
 * 因此我们可以让最后一个字节的数字与6进行相加，再判断是否影响了前面的字节
 * 我们用0x00000010 & ((0xF & x) + 6) 取出第5bit，再右移四位，判断是否为1，如果为1，则说明产生了进位，说明最后的大于9
 */
 ...
 *   需要注意这里的2.0是指10进制下的2.0
 *   因此，x最小为IEEE表示exp部分为全0，即E为1-127,f只有最后一位为1，即2的-23次方，合计2的-149，因此x最小为-149,最大为-127
 *   当x>=-126时，此时IEEE表示，exp部分为1,即1-127,此时2.0的x次幂表示为IEEE下的规格化整数,最大到127 + 127 = 254
 *   当x>127时,IEEE表示下,exp部分为全1(127+128),即表示特殊值  
 *   当x<-149时,2.0^x的e部分为全0(此时为2^-23 * 2^-126)
 */
